...
fn durbin_watson(arr: &[f64]) -> f64 {
    ...
    let sum_sq_diff: f64 = (1..arr.len()).map(|i| (arr[i] - arr[i - 1]).powi(2)).sum();
...
fn biweight_mean(arr: &[f64]) -> f64 {
    ...
    let weights: Vec<f64> = u
        .iter()
        .map(|ui| {
            if ui.abs() < 1.0 {
                (1.0 - ui.powi(2)).powi(2)
            } else {
                0.0
            }
        })
...
fn steady_state_detection(arr: &[f64]) -> (f64, f64, usize) {
    if arr.len() < 10 {
        return (mean(arr), variance(arr), 0);
    }
...
pub fn calculate_stats(history: &[HistoryEntry]) -> Option<StatsResult> {
    ...
    let trend_direction = if offset_reg.slope.abs() < 0.001 {
        "stable"
    } else if offset_reg.slope > 0.0 {
        "up"
    } else {
        "down"
    };
    let delay_trend_direction = if delay_reg.slope.abs() < 0.001 {
        "stable"
    } else if delay_reg.slope > 0.0 {
        "up"
    } else {
        "down"
    };
    ...
    let bias = if consecutive.pos_count > consecutive.neg_count {
        "Positive"
    } else if consecutive.pos_count < consecutive.neg_count {
        "Negative"
    } else {
        "Balanced"
    };
    ...
    Some(StatsResult {
        ...
        drift_rate: if history.len() > 1 {
            (offsets.last().unwrap() - offsets.first().unwrap()) / history.len() as f64
        } else {
            0.0
        },
        ...
        older_avg: mean(
            &offsets
                .iter()
                .take(offsets.len().saturating_sub(10))
                .cloned()
                .collect::<Vec<_>>(),
        ),
        ...
        z_outlier_pct_delay: count_z_score_outliers(&delays, 3.0) as f64 / delays.len() as f64
            * 100.0,
        ...
        crossing_rate: if history.len() > 1 {
            consecutive.crossings as f64 / (history.len() - 1) as f64
        } else {
            0.0
        },
        ...
        within_1ms: abs_offsets.iter().filter(|&&v| v <= 1.0).count() as f64
            / abs_offsets.len() as f64
            * 100.0,
        within_5ms: abs_offsets.iter().filter(|&&v| v <= 5.0).count() as f64
            / abs_offsets.len() as f64
            * 100.0,
        within_10ms: abs_offsets.iter().filter(|&&v| v <= 10.0).count() as f64
            / abs_offsets.len() as f64
            * 100.0,
        within_25ms: abs_offsets.iter().filter(|&&v| v <= 25.0).count() as f64
            / abs_offsets.len() as f64
            * 100.0,
        within_50ms: abs_offsets.iter().filter(|&&v| v <= 50.0).count() as f64
            / abs_offsets.len() as f64
            * 100.0,
        within_100ms: abs_offsets.iter().filter(|&&v| v <= 100.0).count() as f64
            / abs_offsets.len() as f64
            * 100.0,
        within_500ms: abs_offsets.iter().filter(|&&v| v <= 500.0).count() as f64
            / abs_offsets.len() as f64
            * 100.0,
        within_1s: abs_offsets.iter().filter(|&&v| v <= 1000.0).count() as f64
            / abs_offsets.len() as f64
            * 100.0,
        ...
        turning_point_rate: if history.len() > 2 {
            turning_points_count(&offsets) as f64 / (history.len() - 2) as f64
        } else {
            0.0
        },
...
pub fn calculate_autocorr_chart(history: &[HistoryEntry], max_lag: usize) -> Vec<f64> {
    let offsets: Vec<f64> = history.iter().map(|h| h.offset).collect();
    (1..=max_lag)
        .map(|lag| autocorrelation(&offsets, lag))
        .collect()
}
