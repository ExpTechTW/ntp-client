...
const getStatus = (ms: number, t: (key: string) => string) => {
  ...
  if (abs < 10)
    return { color: "text-emerald-500", label: t("home.status.excellent") };
  if (abs < 50)
    return { color: "text-green-500", label: t("home.status.good") };
  if (abs < 100)
    return { color: "text-yellow-500", label: t("home.status.normal") };
  if (abs < 500)
    return { color: "text-orange-500", label: t("home.status.deviation") };
...
const Info = ({
  ...
  >
    ...
    <p
      className={`text-[10px] font-mono leading-tight mt-0.5 ${isDark ? "text-zinc-300" : "text-zinc-700"}`}
    >
      {value}
    </p>
    {sub && (
      <p
        className={`text-[8px] font-mono leading-none mt-0.5 ${isDark ? "text-zinc-600" : "text-zinc-400"}`}
      >
        {sub}
      </p>
    )}
...
export default function HomePage() {
  ...
  const query = async (srv: string) => {
    ...
    try {
      const res = JSON.parse(
        await invoke<string>("sync_ntp_time", { server: srv.trim() }),
      );
      if (res.server) {
        const newResult = {
          success: res.success,
          ...
          ref_id: "",
          ref_time: 0,
          pre_sync_offset: res.pre_sync_offset,
          post_sync_offset: res.post_sync_offset,
        };
        setResult(newResult);
        ...
        setSidecarNotInstalled(
          res.code === "SIDECAR_NOT_INSTALLED" ||
            res.code === "SIDECAR_NOT_RUNNING",
        );
      } else {
        setResult(null);
      }
      ...
      if (autostartEnabled) {
        ...
        if (res.success) {
          const status = JSON.parse(
            await invoke<string>("is_autostart_enabled"),
          );
  ...
  useEffect(() => {
    ...
    const now = Date.now();
    const shouldSkipInitialSync =
      lastSyncTime && now - parseInt(lastSyncTime, 10) < 60000;
      ...
      if (cachedResult) {
        try {
          ...
          const elapsed = Math.floor(
            (now - parseInt(lastSyncTime!, 10)) / 1000,
          );
    ...
    >
      ...
      >
        ...
        >
          ...
          <span
            className={`text-[9px] tabular-nums flex items-center gap-0.5 ${isDark ? "text-zinc-600" : "text-zinc-400"}`}
          >
            <Timer className="w-2.5 h-2.5" />
            {isQuerying ? "--" : `${countdown}s`}
          </span>
          <button
        ...
        >
          {result ? (
            <>
              <div
                className={`flex items-center border-b ${isDark ? "border-zinc-800" : "border-zinc-200"}`}
              >
                <div
                  className={`flex items-center gap-1 px-1.5 py-1 border-r ${isDark ? "border-zinc-800" : "border-zinc-200"}`}
                >
                  {result.success ? (
              ...
              <div className="p-1.5">
                {tab === "time" && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-1">
                    <Info
                      label={t("home.timeLabels.t1Send")}
                      value={fmtTs(result.t1)}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.timeLabels.t2Receive")}
                      value={fmtTs(result.t2)}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.timeLabels.t3Response")}
                      value={fmtTs(result.t3)}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.timeLabels.t4Received")}
                      value={fmtTs(result.t4)}
                      isDark={isDark}
                    />
                  </div>
                )}
                ...
                {tab === "calc" && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-1">
                    <Info
                      label={t("home.calcLabels.offset")}
                      value={
                    ...
                    />
                    <Info
                      label={t("home.calcLabels.delay")}
                      value={fmtS(result.delay)}
                      sub="(T4-T1)-(T3-T2)"
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.calcLabels.rtt")}
                      value={fmtS(result.t4 - result.t1)}
                      sub="T4-T1"
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.calcLabels.processingTime")}
                      value={fmtS(result.t3 - result.t2)}
                      sub="T3-T2"
                      isDark={isDark}
                    />
                  </div>
                ...
                {tab === "packet" && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-1">
                    <Info
                      label={t("home.packetLabels.liVnMode")}
                      value={`0x${((result.leap << 6) | (result.version << 3) | result.mode).toString(16).padStart(2, "0")}`}
                      sub={`LI=${result.leap} VN=${result.version} M=${result.mode}`}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.packetLabels.stratum")}
                      value={result.stratum}
                      sub={getStratumDesc(result.stratum, t)}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.packetLabels.poll")}
                      value={`${result.poll}`}
                      sub={`${Math.pow(2, result.poll)}s`}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.packetLabels.precision")}
                      value={`${result.precision}`}
                      sub={`${Math.pow(2, result.precision).toExponential(1)}s`}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.packetLabels.rootDelay")}
                      value={fmtS(result.root_delay)}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.packetLabels.rootDisp")}
                      value={fmtS(result.root_dispersion)}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.packetLabels.refId")}
                      value={result.ref_id || "-"}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.packetLabels.refTime")}
                      value={result.ref_time ? fmtTs(result.ref_time) : "-"}
                      isDark={isDark}
                    />
                  </div>
                )}
                ...
                {tab === "compare" && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-1">
                    <Info
                      label={t("home.calcLabels.preSyncOffset")}
                      value={
                        <span
                          className={getStatus(result.pre_sync_offset, t).color}
                        >
                          {result.pre_sync_offset >= 0 ? "+" : ""}
                          {fmtS(result.pre_sync_offset)}
                        </span>
                      }
                      sub={t("home.calcSubs.preSyncMeasure")}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.calcLabels.postSyncOffset")}
                      value={
                        <span className={status?.color}>
                          {result.post_sync_offset >= 0 ? "+" : ""}
                          {fmtS(result.post_sync_offset)}
                        </span>
                      }
                      sub={t("home.calcSubs.postSyncVerify")}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.calcLabels.correctionAmount")}
                      value={fmtS(
                        result.pre_sync_offset - result.post_sync_offset,
                      )}
                      sub={t("home.calcSubs.improvement")}
                      isDark={isDark}
                    />
                    <Info
                      label={t("home.calcLabels.delay")}
                      value={fmtS(result.delay)}
                      sub={t("home.calcLabels.networkDelay")}
                      isDark={isDark}
                    />
                  </div>
                )}
            ...
            <div
              className={`flex items-center justify-center gap-1 py-2 ${isDark ? "text-zinc-600" : "text-zinc-400"}`}
            >
        ...
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center gap-3">
            <span
              className={`text-[10px] font-mono ${isDark ? "text-zinc-500" : "text-zinc-500"}`}
            >
              {version}
            </span>
            <label className="flex items-center gap-1.5 cursor-pointer group">
              ...
              <div className="flex items-center gap-1">
                <Power
                  className={`w-3 h-3 ${isDark ? "text-zinc-500 group-hover:text-zinc-400" : "text-zinc-500 group-hover:text-zinc-600"} transition-colors`}
                />
                <span
